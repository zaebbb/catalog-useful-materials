...
import {
  type TechnologyPatternResponse, type TechnologyPatternRequest,
} from '../types/CreateNotePatternTechnologySchema'

export const CreateNotePatternTechnology =
  createAsyncThunk<TechnologyPatternResponse, Files, ThunkConfig<string>>(
    'CreateNote/CreateNotePatternTechnology',
    async (
      files,
      thunkApi
    ) => {
      ...
      const formData: TechnologyPatternRequest = {
        linkTechnology: state.createNotePatternTechnology?.values.linkTechnology ?? '',
        linkDocs: state.createNotePatternTechnology?.values.linkDocs ?? '',
        linkInstall: state.createNotePatternTechnology?.values.linkInstall ?? '',
        icon,
      ...
      try {
        ...
        const response = await extra.api.post<TechnologyPatternResponse>(
          '/notes/create-pattern-technology',
          makeData,
          {
            headers: {
