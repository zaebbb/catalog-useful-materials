...
import {
  type CodePatternResponse, type CodePatternRequest,
} from '../types/CreateNotePatternCodeSchema'

export const CreateNotePatternCode =
  createAsyncThunk<CodePatternResponse, void, ThunkConfig<string>>(
    'CreateNote/CreateNotePatternCode',
    async (
      files,
      thunkApi
    ) => {
      ...
      const formData: CodePatternRequest = {
        code: state.createNotePatternCode?.values.code ?? '',
