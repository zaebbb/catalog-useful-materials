...
import {
  type SaveNotePatternServiceParams,
  type ServicePatternRequest, type ServicePatternResponse,
} from '../types/NotePatternServiceSchema'

export const SaveNotePatternService =
  createAsyncThunk<ServicePatternResponse, SaveNotePatternServiceParams, ThunkConfig<string>>(
    'Note/SaveNotePatternService',
    async (
      params,
      thunkApi
    ) => {
      ...
      const formData: ServicePatternRequest = {
        mode,
        id: state.noteBaseField?.values.id,
        linkService: state.notePatternService?.values.linkService ?? '',
      ...
      try {
        const makeData = makeFormData<ServicePatternRequest>({
          data: formData,
        })

        const response = await extra.api.post<ServicePatternResponse>(
          '/notes/save-pattern-service',
          makeData,
          {
            headers: {
